...
public abstract class CustomObjective implements Listener {
    ...
    public Map<String, Object> getDatamap(Player player, CustomObjective obj) {
        ...
        final Quester quester = Quests.getInstance().getQuester(player.getName());
        if (quester != null) {

            int index = -1;
            int tempIndex = 0;

            for (final me.blackvein.quests.CustomObjective co: quester.currentStage.customObjectives) {
    ...
    public static void incrementObjective(Player player, CustomObjective obj, int count) {
        ...
        if (quester != null) {
            ...
            for (final CustomObjective co: quester.currentStage.customObjectives) {
            ...
            if (hasQuest && quester.hasCustomObjective(obj.getName())) {
                ...
                if (quester.customObjectiveCounts.containsKey(obj.getName())) {
                    final int old = quester.customObjectiveCounts.get(obj.getName());
                    Quests.getInstance().getQuester(player.getName()).customObjectiveCounts.put(obj.getName(), old
                            + count);
                } else {
                ...
                if (index > -1) {
                    ...
                    if (quester.customObjectiveCounts.get(obj.getName()) >= quester.currentStage.customObjectiveCounts
                            .get(index)) {
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof CustomObjective) {
            ...
            final CustomObjective other = (CustomObjective) o;
            ...
            for (final String s: other.datamap.keySet()) {
                if (datamap.containsKey(s) == false) {
                    return false;
                }
            }

            for (final Object val: other.datamap.values()) {
                if (datamap.containsValue(val) == false) {
                    return false;
                }
            }

            for (final String s: other.descriptions.keySet()) {
                if (descriptions.containsKey(s) == false) {
                    return false;
                }
            }

            for (final String s: other.descriptions.values()) {
