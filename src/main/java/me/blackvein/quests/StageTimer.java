...
public class StageTimer implements Runnable {
    ...
    public void run() {
        ...
        if (quester.delayOver) {
            ...
            final Player player = plugin.getServer().getPlayerExact(quester.name);

            if (quester.currentQuest != null) {

                if (quester.currentQuest.orderedStages.indexOf(quester.currentStage) == (quester.currentQuest.orderedStages
                        .size() - 1)) {
